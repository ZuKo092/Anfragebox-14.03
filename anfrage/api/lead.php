...
log_info('LEAD MAP | slug=' . $incomingSlug . ' | incoming_company_id=' . $incomingCompanyId . ' | resolved_company_id=' . $resolvedCompanyId);
...
log_info('INSERT_ROW=' . json_encode($insertRow, JSON_UNESCAPED_UNICODE));
...
if ($leadId !== '' && $customer_email !== '') {
  ...
  if ($ptHttp >= 200 && $ptHttp < 300) {
    ...
    log_info('Portal token created for lead ' . $leadId . ': ' . $portalToken);
  } else {
...
log_info('FINAL INSERT company_id=' . ($insertRow['company_id'] ?? 'NULL'));
...
$notifyBase = NOTIFY_INTERNAL_BASE_URL;
if ($notifyBase === '') {
  $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
  $host = $_SERVER['HTTP_HOST'] ?? '127.0.0.1';
  $notifyBase = $scheme . '://' . $host;
}
$notifyUrl = rtrim($notifyBase, '/') . '/anfrage/api/notify-new-lead.php';
